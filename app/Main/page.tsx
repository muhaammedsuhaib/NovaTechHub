...
const Main: React.FC = () => {
  ...
  return (
    <>
      <div
        className={`w-full h-[100vh] ${mode ? "bg-white text-black" : "bg-black text-white"}`}
      >
        <Navbar />
        <LogoSlider />
        ...
        <div className="text-center mx-auto">
          ...
          <h6 className={`${styles.title1} font-bold`}>
            At the Premier Web Development Hub
          </h6>
          <form onSubmit={handleSubmit} className="relative w-72 mx-auto">
            <div
              className={`flex items-center ${mode ? "bg-white text-black" : "bg-black text-white"}`}
            ...
            {searchTerm && (
              <ul
                className={`absolute z-10 w-full h-[200px] overflow-auto text-start ${styles.suggetion}`}
              >
          ...
          <a href="/Tutorial">
            <h1 className={`underline mt-10 font-bold ${searchTerm && "hidden"}`}>
